...
for year=2026:2026
   for day=1:365
      ...
      if (exist(['out',num2str(year),'_',num2str(day,"%03d")])==0)
         nomENSMM=['A48800FRA_S_',num2str(year),num2str(day,'%03d'),'0000_01D_30S_MO.crx'];
         % https://renag.resif.fr/pub/centipede_30s/2024/088/ENSM00FRA_S_20240880000_01D_30S_MO.crx.gz
         % https://renag.resif.fr/pub/centipede_30s/2026/117/A09000CHZ_S_20261170000_01D_30S_MO.crx.gz
	 if (exist(strrep(nomENSMM,'.gz',''))==0)
           system(['wget https://renag.resif.fr/pub/centipede_30s/',num2str(year),'/',num2str(day,"%03d"),'/',nomENSMM,'.gz']);
	 ...
	 if (exist(strrep(nomENSMM,'.gz',''))==0)
           nomENSMM=['ENSM00FRA_S_',num2str(year),num2str(day,'%03d'),'0000_01D_30S_MO.crx'];
           system(['wget https://renag.resif.fr/pub/centipede_30s/',num2str(year),'/',num2str(day,"%03d"),'/',nomENSMM,'.gz']);
           system(['gunzip -f ',nomENSMM,'.gz']);
           system(['../RNXCMP_4.2.0_src/source/crx2rnx -f ',nomENSMM]);
           system(['../RTKLIB_2.4.3/app/consapp/convbin/gcc/convbin ',strrep(nomENSMM,'crx','rnx')]);
