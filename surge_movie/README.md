...
4. to assemble PNG to a movie
```
ffmpeg -f image2 -r 25 -pattern_type glob -i '*HH.png' -vcodec libx264 -crf 22 video.mp4
```
5. to shrink the PNG size in order to generate a more modest movie
```
for i in 2*HH.png;do nom=`echo $i | sed 's/\.png/small\.png/g'` ; d=`echo $i | cut -c 1-8`;echo $i $nom; convert -crop 1750x1200+1200+1000 -fill red -stroke red -pointsize 70 -annotate +1210+2065 $d $i $nom; done
ffmpeg -f image2 -r 25 -pattern_type glob -i '*small.png' -vcodec libx264 -crf 22 surgesmall.mp4
```
